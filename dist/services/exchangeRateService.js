...
class ExchangeRateService {
    ...
    static async fetchLatestRates(baseCurrency = 'USD') {
        try {
            const response = await axios_1.default.get(`${this.API_BASE_URL}/latest/${baseCurrency}`, {
    ...
    static async getCachedRates(baseCurrency) {
        try {
            const oneHourAgo = new Date(Date.now() - this.CACHE_DURATION);
            const cachedRates = await database_1.default.exchangeRate.findMany({
                where: {
    ...
    static async cacheRates(baseCurrency, rates) {
        try {
            // Delete old rates for this base currency
            await database_1.default.exchangeRate.deleteMany({
                where: {
                    fromCurrency: baseCurrency,
                },
            });
            ...
            const filteredRates = Object.entries(rates).filter(([currency]) => supportedCurrencies.includes(currency));
            // Insert new rates
            ...
            if (rateRecords.length > 0) {
                await database_1.default.exchangeRate.createMany({
                    data: rateRecords,
                });
            }
        }
    ...
    static getCurrencyInfo(currencyCode) {
        return (this.CURRENCY_INFO[currencyCode] || {
            flag: '🏳️',
            symbol: currencyCode,
            name: currencyCode,
            country: 'Unknown',
    ...
    static async getRateUpdateStatus() {
        try {
            const latestRate = await database_1.default.exchangeRate.findFirst({
                orderBy: { timestamp: 'desc' },
            });
            const totalRates = await database_1.default.exchangeRate.count();
            return {
...
ExchangeRateService.API_BASE_URL = 'https://api.exchangerate-api.com/v4';
ExchangeRateService.CACHE_DURATION = 60 * 60 * 1000; // 1 hour in milliseconds
// Comprehensive currency information
ExchangeRateService.CURRENCY_INFO = {
    // Major currencies
