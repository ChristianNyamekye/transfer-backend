...
import { CreateUserRequest, LoginRequest, LoginResponse } from '@/types/auth';
...
export class AuthController {
  ...
  static async getAccountData(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user) {
        const response: ApiResponse = {
          success: false,
      ...
      const currencyInfo: Record<string, { flag: string; symbol: string; name: string }> = {
